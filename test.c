...
void readDeg(FILE *input, char *name)
{
    ...
    while (!feof(input))
    {
        ...
        if (strncmp(preReqs, "OR", 2) == 0)
        {
            ...
            token = strtok(preReqs, ",");
            token = cleanInput(token);
            CourseLinkedList *cl = createCourseLinkedList();
            while (token != NULL)
            {
                token = cleanInput(token);
                printf("%s\n", token);
                insertCourseLinkedList(cl, token);
                token = strtok(NULL, ",");
            }
            insertDegreeReq(degReq, cl);
        }
        else
        {
            CourseLinkedList *cl2 = createCourseLinkedList();
            insertCourseLinkedList(cl2, token);
            insertDegreeReq(degReq, cl2);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    fileInput("Test-dep.txt");
    fileInput("Test-deg.txt");
    //printDeptArrayList(deptArray);
    //char *testCourse = deptArray->list[0]->courses->list[0]->code;
    //printf("%s\n", testCourse);
    //printf("%d\n", strcmp(testCourse, "Psych 110"));
    //char *testDeg = degreeArray->list[0]->name;
    //testDeg = strtok(testDeg, "\n");
    //testDeg[strcspn(testDeg, "\r\n")] = 0;
    //printf("%d\n", strcmp(testDeg, "BS Psychology"));
    //commandC("Psych 110");
    //commandD("BS Psychology");
}